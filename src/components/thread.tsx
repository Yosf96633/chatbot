...
import { ToolCallUI } from "@/components/tool-call-ui";
...
const AssistantMessage: FC = () => {
  const toolCalls = useAuiState(
    (s) => (s.message.metadata?.custom as { toolCalls?: ToolCall[] } | undefined)?.toolCalls ?? [],
  );
  ...
  return (
    ...
    >
      <div className="aui-assistant-message-content wrap-break-word px-2 text-foreground leading-relaxed">
        <ToolCallUI toolCalls={toolCalls} />
        ...
        <AuiIf condition={(s) => s.thread.isRunning && s.message.isLast}>
          <span className="mt-2 block size-2 rounded-full bg-foreground/50 animate-[breathe_1.2s_ease-in-out_infinite]" />
        </AuiIf>
        <MessageError />
      </div>
